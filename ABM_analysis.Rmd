...
y_linpred_drip <- posterior_linpred(model31, newdata = test_data)
hist(y_linpred_drip, main = "Histogram of predicted value for Transitivity")
preds <- tibble(pred=predict(model31, new_data = test_data, type = "response"))


ggplot() +
  geom_density(aes(x = pred), data=preds,adjust = 1L, fill = "#112446") +
  theme_minimal()+
  geom_density(aes(x=trans),data=train_data)
```
...
for (row in 1:nrow(df_collected)){
    ...
    temp <- tibble(moves = stat$degree[[row]]$`degree(post_igraph, mode = "all")`,
                   table = stat$table[row],
                   iteration = stat$iteration[row],
                   net = stat$network[row])
}
