...
```{r friends network}
#Add connection to weight
#works fine but does not save the weight to the pre_network
for (i in 1:nrow(party)){
  current_id <- party$ID[i]
  temp <- pre_network %>% filter(from==current_id)
  temp2 <- temp[temp$to==party$friend_ids[i]]
   for (k in party$friend_id[i]){
...
temp <- list()
for (i in guests$ID){
 n_friends <- filter(guests, ID==i) %>% select(missing_friends) %>% as.numeric()
 possible_friends <- guests$ID[guests$ID !=i & guests$ID <100]
 friends <- sample(possible_friends, n_friends, replace = F)
 friends <- dataframe
 temp <- rbind(temp, friends)
}
```
...
#does not work for more than first row...
for (i in 1:nrow(party)) {
  for (k in 1:length(party$friend_id[[i]])) {
    j <- as.numeric(party$friend_id[[i]][[k]])
    matrix[i, j] <- 16
  }
}
matrix
