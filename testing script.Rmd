...
df_collected <- tibble(expand.grid(iteration=seq(iterations), # Create an empty list to store the data frames
                                   table_type,party=vector("list", length = iterations),
...
```{r long}
#long
long_table <- tibble(table=NA,seat=1:n_people,contact=NA,indices=NA)
...
for (p in table_type){
  ...
  for (i in 1:iterations){
    ...
    df_collected$table_type[i] <- paste(table_type)
    ...
    #start party
    for (tic in 1:tics){
      #seat everybody at the table
      #calculate proximity
      #evalute satisfaction for proximity
      ...
      #evalute satisfaction for contacts seats
      #evalute overall satisfaction
      #decide whether to move
        if (decision_move == 'yes'){
          no_person <- c(ID=party$id[that_person_index])
          no_seat_list <- rbind(no_seat_list,no_person)
          #remove that person from their current table by setting ID to NA
        } #else stay seated
