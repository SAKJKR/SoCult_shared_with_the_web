...
make_hosts <- function(number_of_iterations){ 
  ...
  hosts <<- tibble(
      ...
      moves = NA #for counting how many times they move
    )
}
``` 
...
for (p in table_type){
  ...
  for (i in 1:iterations){
    ...
    #make prenetwork
    pre_network <- make_prenetwork(party)
    #save prenetwork
    ...
    for (tic in 1:tics){
      ...
      #calculate proximity
        #check whether peoples current friends are near
        #(meaning for round & square whether they are at the table, long whether they are within contact seats)
        ...
        friends_within_reach <- 0 #set counter
        cs_len <- as.vector(table$indices[person])
        fr_len <- length(prox_friends)
        #check overlap with contact seats
        for (fr in seq(prox_friends)){
          #kommet her til
