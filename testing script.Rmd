...
make_party <- function(index_of_current_iteration){
  ...
  party <<- party %>% 
    mutate(friend_id=NA) %>% 
    mutate(satisfaction_baseline = 0) %>% # not simulated yet, CANNOT BE ZERO
...
for (p in table_type){
  ...
  for (i in 1:iterations){
    df_collected$table_type[i] <- if (p == 1) "long" else if (p == 2) "round" else "square"
    
    print("starting ", i, " out of ", iterations, " iterations for ", df_collected$table_type[i], " type of table")
    ...
    #start party
    for (tic in 1:tics){
    print("starting ", tic, " out of ", tics, " tics ")
      ...
      #evalute overall satisfaction
          party[person_party, "satisfaction"][[1]] <- party[person_party, "satisfaction"][[1]] + check_friends_nearby #guests satisfaction is updated with the percentage of the people that they can sit next to actually are their friends (have a weight above 0) currently
      ...
      #decide whether to move
        ...
        # Convert values to yes(1) or no(0) answer
        decision_move <- ifelse(change > threshold, "no", "yes")
      ...
      party$satisfaction <-  party$satisfaction_baseline
      
      
    } #after all tics
    
    #save postnetwork
    df_collected$post_network[[i]] <- network
    #save party
...
minutes <- time_diff$min
seconds <- time_diff$sec

# Print the time difference
print("It took ", hours, "H ", minutes, "M ", seconds, "S", "to run")
