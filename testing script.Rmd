  ...
  for (i in 1:iterations){
    ...
    for (tic in 1:tics){
      ...
      #look at contact seats
        ...
        for (cp in 1:length(con_people)){
              #in n_interest
          row2 <- which(party$ID==con_people[cp])
          ...
          relation_possibilities$outcome[cp] <- wf *
            relation_possibilities$friendshipscore[cp] + wa *
        ...
        relation_possibilities$commitment[cp] <- relation_possibilities$outcome[cp] + (iterations - i +1) - max(relation_possibilities$outcome, na.rm =TRUE) 
      ...
      #assuming person index = person, no_seat list exists
        if (decision_move == 'yes'){
          ...
          party[person, "moves"][[1]] <- party[person, "moves"][[1]] + 1
        row_index <- which(table$ID == person)
          #remove from their seat
          table$ID[row_index] <- 0
          #add to no seat
